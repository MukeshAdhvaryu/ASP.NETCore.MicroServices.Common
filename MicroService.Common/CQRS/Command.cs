...
namespace MicroService.Common.CQRS
{
    ...
    public class Command<TOutDTO, TModel, TID> : ICommand<TOutDTO, TModel, TID>, IFirstModel<TModel>
    ...
    {
        #region VARIABLES
        readonly IExCommand<TID, TModel> command;
        ...
        #region CONSTRUCTORS
        public Command(IModelContext _context) :
            this(_context.CreateCommand<TID, TModel>())
        { }
        public Command(ICommand<TID, TModel>? _command)
        {
            if (!(_command is IExCommand<TID, TModel>))
            {
                throw DummyModel.GetModelException(ExceptionType.InvalidContext);
            }
            command = (IExCommand<TID, TModel>)_command;
        }
        #endregion

        #region PROPERTIES
        protected ICommand<TID, TModel> InnerCommand => command;
        #endregion
        ...
        public async virtual Task<TOutDTO?> Update(TID id, IModel? model)
        {
            ...
            var result = await command.Get(id);
            if (result == null)
                throw DummyModel.GetModelException(ExceptionType.NoModelFoundForID, id.ToString());
    ...
    /// <typeparam name="TModel">Model of your choice.</typeparam>
    /// <typeparam name="TID">Primary key type of the model.</typeparam>
    public class Command<TModel, TID> : Command<TModel, TModel, TID>
        #region TYPE CONSTRINTS
        where TModel : class, ISelfModel<TID, TModel>, new()
        //-:cnd:noEmit
#if (!MODEL_USEDTO)
        ...
        #endregion
    {
        public Command(IModelContext _context) : base(_context)
        {
        }
        public Command(ICommand<TID, TModel>? _command) : base(_command) 
        { }

        protected sealed override TModel? ToDTO(TModel? model) =>
            model;
        protected sealed override IEnumerable<TModel>? ToDTO(IEnumerable<TModel>? models) =>
            models;
    }
    #endregion
}
