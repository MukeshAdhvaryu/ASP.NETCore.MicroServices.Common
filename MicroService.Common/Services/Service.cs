...
namespace MicroService.Common.Services
{
        ...
        #endregion
    {
        ...
        IQuery<TOutDTO, TModel, TID> Query;
#endif
#if MODEL_APPENDABLE || MODEL_UPDATABLE || MODEL_DELETABLE
        ICommand<TOutDTO, TModel, TID> Command;
#endif
        //+:cnd:noEmit
        #endregion
        ...
        public Service(TContext _context)
        {
            ...
            goto CREATEQUERY;
#else
            var command = _context.CreateCommand<TID, TModel>();
            Command = new Command<TOutDTO, TModel, TID>(command);
#if !MODEL_NONREADABLE || !MODEL_NONQUERYABLE
            Query = new Query<TOutDTO, TModel, TID>(((IExCommand<TID, TModel>)command).GetQueryObject());
            return;
#endif
#endif
            CREATEQUERY:
#if !MODEL_NONREADABLE || !MODEL_NONQUERYABLE
            Query = new Query<TOutDTO, TModel, TID>(_context);
#endif
            return;
            //+:cnd:noEmit
        }
        ...
        //-:cnd:noEmit
#if (MODEL_APPENDABLE || MODEL_UPDATABLE || MODEL_DELETABLE)
        public Service(ICommand<TID, TModel> command)
        {
            Command = new Command<TOutDTO, TModel, TID>(command);
#if !MODEL_NONREADABLE || !MODEL_NONQUERYABLE
            Query = new Query<TOutDTO, TModel, TID>(((IExCommand<TID, TModel>)command).GetQueryObject());
